...
from .core import Chartable
from .core import DataLoader, load_config, DEFAULT_CONFIG
...
class FundamentalsAnalysis(Chartable):
    ...
    def load(self):
        ...
        print(f'[Fundamentals] US stocks {len(self.us_stocks)}w | '
    ...
    def summary_stats(self):
        ...
        for c in ['Crude_incl_SPR_Mbbl', 'SPR_Crude_Stocks_Mbbl',
                  'Gasoline_Stocks_Mbbl', 'Distillate_Stocks_Mbbl',
                  'Total_Products_Mbbl']:
        ...
        fs5 = self.floating[self.floating['Date'] >= self.floating['Date'].max() - pd.DateOffset(years=5)]
        print(f'Floating storage: latest = {self.floating.iloc[-1]["Floating_Storage_kt"]:,.0f} kt, '
    ...
    def chart_us_stocks(self):
        ...
        fig, axes = plt.subplots(3, 1, figsize=(self.SIZES['STANDARD'][0], 10),
                                 sharex=True)
        panels = [
        ...
        for ax, (col, title) in zip(axes, panels):
            ax.plot(df5['Date'], df5[col], color=self.COLORS['STOCKS'],
                    linewidth=self.LW)
            # 5y percentile band
            p20 = df5[col].quantile(0.20)
            p80 = df5[col].quantile(0.80)
            ax.axhline(p20, color=self.COLORS['REF'], linestyle='--',
                       linewidth=0.7, alpha=0.6, label=f'20th pct = {p20:,.0f}')
            ax.axhline(p80, color=self.COLORS['REF'], linestyle='--',
                       linewidth=0.7, alpha=0.6, label=f'80th pct = {p80:,.0f}')
            ax.set_ylabel(f'{title}\n(Mbbl)')
            ax.legend(loc='upper left', fontsize=8)
            ax.set_title(title, loc='left', fontsize=10, fontweight='normal')
        self._format_xaxis(axes[-1])
        axes[-1].set_xlabel('Date')
        fig.suptitle('US petroleum stocks — last 5 years (EIA weekly)',
    ...
    def chart_spr_drawdown(self):
        ...
        ax_top.plot(since_2021['Date'], since_2021['SPR_Crude_Stocks_Mbbl'],
                    color=self.COLORS['SPR'], linewidth=self.LW + 0.3)
        ax_top.axhline(peak, color=self.COLORS['REF'], linestyle='--',
                       linewidth=0.8, label=f'Peak {peak:.0f} Mbbl ({peak_date.date()})')
        ax_top.axhline(trough, color=self.COLORS['REF'], linestyle='--',
                       linewidth=0.8, label=f'Trough {trough:.0f} Mbbl ({trough_date.date()})')
        ax_top.set(ylabel='SPR crude (Mbbl)')
        ax_top.set_title('Strategic Petroleum Reserve — level', loc='left',
        ...
        ax_bot.bar(since_2021['Date'], since_2021['weekly_change'],
                   color=np.where(since_2021['weekly_change'] < 0,
                                  self.COLORS['VOL'], self.COLORS['MARGIN']),
                   width=4.5)
        self._zero_line(ax_bot)
        ax_bot.set(xlabel='Date', ylabel='Weekly Δ (Mbbl)')
        ax_bot.set_title('Weekly change — draws (red) vs builds (green)',
                         loc='left', fontsize=10, fontweight='normal')
        self._format_xaxis(ax_bot)

        drawdown_pct = 100 * (latest['SPR_Crude_Stocks_Mbbl'] - peak) / peak
        print(f'SPR peak-to-latest: {latest["SPR_Crude_Stocks_Mbbl"] - peak:+.1f} Mbbl '
              f'({drawdown_pct:+.1f}%)')
        print(f'Latest: {latest["SPR_Crude_Stocks_Mbbl"]:.1f} Mbbl on {latest["Date"].date()}')

        fig.suptitle('SPR drawdown — 2021 → latest',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_eu_vs_us_stocks(self):
        ...
        us = self.us_stocks[['Date', 'Distillate_Stocks_Mbbl',
                              'Gasoline_Stocks_Mbbl']].copy()
        ...
        ax.plot(us['Date'], z(us['Distillate_Stocks_Mbbl']),
                color=self.COLORS['CRACK'], linewidth=self.LW,
                label='US distillate (z)')
        ax.plot(us['Date'], z(us['Gasoline_Stocks_Mbbl']),
                color=self.COLORS['MARGIN'], linewidth=self.LW,
                label='US gasoline (z)')
        ax.plot(eu['Date'], z(eu['Euro_All_Oil_Products_Stocks_kt']),
    ...
    def chart_refinery_util(self):
        ...
        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        ax.plot(df['Date'], df['Refinery_Utilization_Pct'],
                color=self.COLORS['PRICE'], linewidth=self.LW + 0.3,
                marker='o', markersize=3)
        ax.axhline(p80, color=self.COLORS['VOL'], linestyle='--',
                   linewidth=0.8, label=f'80th pct = {p80:.1f}%  (tight)')
        ax.axhline(p50, color=self.COLORS['REF'], linestyle='--',
                   linewidth=0.8, label=f'Median = {p50:.1f}%')
        ax.axhline(p20, color=self.COLORS['MARGIN'], linestyle='--',
                   linewidth=0.8, label=f'20th pct = {p20:.1f}%  (slack)')
        ax.set(xlabel='Date', ylabel='Refinery utilization (%)')
        ax.set_title('US refinery utilisation — 2019 onwards (EIA monthly)',
                     loc='left')
        ax.legend(loc='lower left')
        self._format_xaxis(ax)
        latest = df.iloc[-1]
        pct_rank = (df['Refinery_Utilization_Pct'] < latest['Refinery_Utilization_Pct']).mean() * 100
        print(f'Latest utilisation: {latest["Refinery_Utilization_Pct"]:.1f}% '
    ...
    def chart_floating_storage(self):
        ...
        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        ax.plot(df['Date'], df['Floating_Storage_kt'],
                color=self.COLORS['FLOW'], linewidth=self.LW + 0.3,
                marker='o', markersize=3)
        mean = df['Floating_Storage_kt'].mean()
        ax.axhline(mean, color=self.COLORS['REF'], linestyle='--',
                   linewidth=0.8, label=f'Mean = {mean:,.0f} kt')
        ax.set(xlabel='Date', ylabel='Floating storage (kt)')
        ax.set_title('Diesel / gasoil floating storage — monthly (Vortexa/Kpler)',
                     loc='left')
        ax.legend(loc='upper left')
        self._format_xaxis(ax)
    ...
    def chart_eu_stocks_by_country(self, TOP_N=8):
        ...
        top = (df.groupby('Country')['Value'].mean()
                 .sort_values(ascending=False).head(TOP_N).index)
        df_top = df[df['Country'].isin(top)]
        pivot = (df_top.pivot_table(index='Date', columns='Country',
        ...
        ax.set_title('EU petroleum-products stocks by country — Eurostat monthly',
                     loc='left')
    ...
    def chart_stocks_crack_correlation(self):
        ...
        merged = pd.merge_asof(
            us.sort_values('Date'), crack.sort_values('Date'),
            on='Date', direction='nearest', tolerance=pd.Timedelta('7 days')
        ).dropna()
        five_y = merged['Date'].max() - pd.DateOffset(years=5)
        m5 = merged[merged['Date'] >= five_y]

        fig, (ax_ts, ax_sc) = plt.subplots(
            1, 2, figsize=(self.SIZES['STANDARD'][0] + 2, self.SIZES['SQUARE'][1]),
            gridspec_kw={'width_ratios': [2, 1], 'wspace': 0.3})

        # Time series
        ax_ts.plot(m5['Date'], m5['Distillate_Stocks_Mbbl'],
                   color=self.COLORS['STOCKS'], linewidth=self.LW,
                   label='US distillate stocks')
        self._color_stack(ax_ts, 'y', self.COLORS['STOCKS'], 'Distillate stocks (Mbbl)')
        ax2 = ax_ts.twinx()
        ax2.plot(m5['Date'], m5['Brent_ULSD_NWE_Crack'],
                 color=self.COLORS['CRACK'], linewidth=self.LW, alpha=0.85,
                 label='Brent-ULSD NWE crack')
        self._color_stack(ax2, 'y', self.COLORS['CRACK'], 'Crack (USD/bbl)')
        self._format_xaxis(ax_ts)
        ax_ts.set_title('Distillate stocks vs diesel crack (5y weekly)',
                        loc='left', fontsize=10)

        # Scatter
        ax_sc.scatter(m5['Distillate_Stocks_Mbbl'], m5['Brent_ULSD_NWE_Crack'],
                      s=14, alpha=self.A['SCATTER'], color=self.COLORS['SCATTER'],
                      edgecolor='none')
        if len(m5) > 2:
            slope, intercept = np.polyfit(m5['Distillate_Stocks_Mbbl'],
                                          m5['Brent_ULSD_NWE_Crack'], 1)
            xs = np.array([m5['Distillate_Stocks_Mbbl'].min(),
                           m5['Distillate_Stocks_Mbbl'].max()])
            ax_sc.plot(xs, slope*xs + intercept, color=self.COLORS['TREND'],
                       linewidth=self.LW)
            r = m5[['Distillate_Stocks_Mbbl','Brent_ULSD_NWE_Crack']].corr().iloc[0,1]
            ax_sc.set_title(f'r = {r:+.3f}', loc='left', fontsize=10)
            print(f'Corr distillate stocks vs Brent-ULSD crack: {r:+.3f}  '
                  f'(n={len(m5)})')
        ax_sc.set(xlabel='Distillate stocks (Mbbl)', ylabel='Crack (USD/bbl)')
        ax_sc.grid(True, alpha=0.3)

        fig.suptitle('Fundamentals ↔ margin link',
    ...
    def chart_us_util_long(self):
        ...
        ax_top.plot(df['Date'], df['Refinery_Utilization_Pct'],
                    color=self.COLORS['PRICE'], linewidth=self.LW - 0.2, alpha=0.85)
        full_p80 = df['Refinery_Utilization_Pct'].quantile(0.80)
        full_p20 = df['Refinery_Utilization_Pct'].quantile(0.20)
        ax_top.axhline(full_p80, color=self.COLORS['VOL'], linestyle='--',
                       linewidth=0.8, label=f'Full-history 80th = {full_p80:.1f}%')
        ax_top.axhline(full_p20, color=self.COLORS['MARGIN'], linestyle='--',
                       linewidth=0.8, label=f'Full-history 20th = {full_p20:.1f}%')
        ax_top.set(ylabel='Utilisation (%)')
        ax_top.set_title('US refinery utilisation — full EIA weekly history (1982 → latest)',
                         loc='left', fontsize=10, fontweight='normal')
        ax_top.legend(loc='lower left', fontsize=9)
        self._format_xaxis(ax_top)

        ax_bot.plot(df5['Date'], df5['Refinery_Utilization_Pct'],
                    color=self.COLORS['PRICE'], linewidth=self.LW + 0.2,
                    marker='o', markersize=2)
        p20, p80 = df5['Refinery_Utilization_Pct'].quantile([0.20, 0.80])
        ax_bot.fill_between(df5['Date'], p20, p80, color=self.COLORS['REF'],
                            alpha=0.15, label=f'5y 20-80 pct ({p20:.1f}..{p80:.1f}%)')
        ax_bot.set(xlabel='Date', ylabel='Utilisation (%)')
        ax_bot.set_title('Last 5 years — zoom', loc='left',
                         fontsize=10, fontweight='normal')
        ax_bot.legend(loc='lower left', fontsize=9)
        self._format_xaxis(ax_bot)

        latest = df.iloc[-1]
        full_rank = (df['Refinery_Utilization_Pct'] < latest['Refinery_Utilization_Pct']).mean() * 100
        recent_rank = (df5['Refinery_Utilization_Pct'] < latest['Refinery_Utilization_Pct']).mean() * 100
        print(f'Latest: {latest["Refinery_Utilization_Pct"]:.1f}% on {latest["Date"].date()}')
        print(f'Rank vs full history (43y): {full_rank:.0f}th pct  |  vs last 5y: {recent_rank:.0f}th pct')

        fig.suptitle('US refinery utilisation — long-history context',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_us_yields(self):
        ...
        main_cols = ['Yield_Gasoline_pct', 'Yield_Distillate_pct',
                     'Yield_Jet_pct', 'Yield_HGL_pct']
        labels = ['Gasoline', 'Distillate (diesel+gasoil)', 'Jet', 'LPG / NGLs']
        colors = [self.COLORS['MARGIN'], self.COLORS['CRACK'],
                  self.COLORS['PRICE'], self.COLORS['FX']]

        fig, (ax_top, ax_bot) = plt.subplots(
            2, 1, figsize=(self.SIZES['STANDARD'][0], self.SIZES['STANDARD'][1] + 3),
            gridspec_kw={'height_ratios': [2, 1], 'hspace': 0.3})

        for col, lbl, clr in zip(main_cols, labels, colors):
            ax_top.plot(df['Date'], df[col], color=clr, linewidth=self.LW,
                        label=lbl, alpha=0.9)
        ax_top.set(ylabel='Yield (% of refinery input)')
        ax_top.set_title('EIA US refinery yields — 2010 → latest',
                         loc='left', fontsize=10, fontweight='normal')
        ...
        ax_bot.plot(df['Date'], df['distillate_tilt'],
                    color=self.COLORS['CRACK'], linewidth=self.LW + 0.2)
        self._zero_line(ax_bot)
        mean_tilt = df['distillate_tilt'].mean()
        ax_bot.axhline(mean_tilt, color=self.COLORS['REF'], linestyle=':',
                       linewidth=0.8, label=f'Mean = {mean_tilt:+.1f} pp')
        ax_bot.set(xlabel='Date',
                   ylabel='Distillate − gasoline (pp)')
        ax_bot.set_title('Distillate tilt — negative = gasoline-heavy run, '
                         'positive = diesel-heavy run',
                         loc='left', fontsize=10, fontweight='normal')
        ax_bot.legend(loc='upper left', fontsize=9)
        self._format_xaxis(ax_bot)

        latest = df.iloc[-1]
        print(f'Latest yields ({latest["Date"].date()}): '
              f'gasoline {latest["Yield_Gasoline_pct"]:.1f}% | '
              f'distillate {latest["Yield_Distillate_pct"]:.1f}% | '
              f'jet {latest["Yield_Jet_pct"]:.1f}%')
        print(f'Distillate tilt: {latest["distillate_tilt"]:+.1f} pp '
              f'(mean {mean_tilt:+.1f}, σ {df["distillate_tilt"].std():.2f})')

        fig.suptitle('US refinery yield mix',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_eu_refinery_intake(self, TOP_N=8, MIN_COUNTRIES_REPORTING=10):
        ...
        top = (df.groupby('Country')['Value_kt'].mean()
                 .sort_values(ascending=False).head(TOP_N).index)
        df_top = df[df['Country'].isin(top)]
        pivot = (df_top.pivot_table(index='Date', columns='Country',
                                     values='Value_kt', aggfunc='sum')
                       .fillna(0))
        ...
        ax_stack.stackplot(pivot.index,
                           *[pivot[c].values for c in pivot.columns],
                           labels=list(pivot.columns),
                           colors=plt.cm.tab10.colors[:len(pivot.columns)],
                           alpha=0.85, linewidth=0)
        ax_stack.set(ylabel='kt / month (top-%d countries)' % TOP_N)
        ax_stack.set_title('EU refinery crude intake — top contributors (Eurostat monthly observed)',
                           loc='left', fontsize=10, fontweight='normal')
        ax_stack.legend(loc='upper left', ncol=2, fontsize=8)
        self._format_xaxis(ax_stack)

        ax_total.plot(total.index, total.values,
                      color=self.COLORS['PRICE'], linewidth=self.LW,
                      label='EU total (all reporting countries)')
        ax_total.plot(total.index, total.rolling(12).mean(),
                      color=self.COLORS['CRACK'], linewidth=self.LW + 0.2,
                      label='12-month MA')
        ax_total.set(xlabel='Date', ylabel='kt / month')
        ax_total.set_title('EU total refinery intake — level + trend',
                           loc='left', fontsize=10, fontweight='normal')
        ...
        print(f'EU total intake latest ({total.index[-1].date()}): {latest:,.0f} kt/month '
        ...
        fig.suptitle('EU refinery runs — crude intake (observed)',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_eu_distillate_vs_crack(self):
        ...
        out = (self.eu_distillate_out.groupby('Date', as_index=False)['Value_kt'].sum()
        ...
        ax_ts.plot(merged['Date'], merged['EU_distillate_out_kt'],
                   color=self.COLORS['MARGIN'], linewidth=self.LW,
                   label='EU distillate output (kt/m)')
        self._color_stack(ax_ts, 'y', self.COLORS['MARGIN'], 'EU distillate output (kt/m)')
        ax2 = ax_ts.twinx()
        ax2.plot(merged['Date'], merged['Brent_ULSD_NWE_Crack'],
                 color=self.COLORS['CRACK'], linewidth=self.LW, alpha=0.85,
                 label='Brent-ULSD NWE crack')
        self._color_stack(ax2, 'y', self.COLORS['CRACK'], 'Crack (USD/bbl)')
        ax_ts.set_title('EU distillate output vs diesel crack — monthly',
                        loc='left', fontsize=10)
        self._format_xaxis(ax_ts)

        ax_sc.scatter(merged['Brent_ULSD_NWE_Crack'], merged['EU_distillate_out_kt'],
                      s=18, alpha=self.A['SCATTER'], color=self.COLORS['SCATTER'],
                      edgecolor='none')
        if len(merged) > 2:
            r = merged[['EU_distillate_out_kt','Brent_ULSD_NWE_Crack']].corr().iloc[0,1]
            slope, intercept = np.polyfit(merged['Brent_ULSD_NWE_Crack'],
                                          merged['EU_distillate_out_kt'], 1)
            xs = np.array([merged['Brent_ULSD_NWE_Crack'].min(),
                           merged['Brent_ULSD_NWE_Crack'].max()])
            ax_sc.plot(xs, slope*xs + intercept, color=self.COLORS['TREND'],
                       linewidth=self.LW)
            ax_sc.set_title(f'r = {r:+.3f}  (n={len(merged)})',
                            loc='left', fontsize=10)
            print(f'Corr EU distillate output vs Brent-ULSD crack: {r:+.3f}  '
                  f'(n={len(merged)})')
        ax_sc.set(xlabel='Crack (USD/bbl)', ylabel='EU distillate output (kt/m)')
        ax_sc.grid(True, alpha=0.3)

        fig.suptitle('EU refineries — margin signal → output response',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=1.02)
    ...
    def chart_eu_consumption(self, MIN_COUNTRIES_REPORTING=15):
        ...
        merged = cons_tot.merge(out_tot, on='Date', how='inner',
                                suffixes=('_consumption', '_output'))
        ...
        ax_top.plot(merged['Date'], merged['Value_kt_consumption'],
                    color=self.COLORS['CRACK'], linewidth=self.LW + 0.2,
                    label='EU consumption (gross inland deliveries)')
        ax_top.plot(merged['Date'], merged['Value_kt_output'],
                    color=self.COLORS['MARGIN'], linewidth=self.LW + 0.2,
                    label='EU refinery output')
        ax_top.set(ylabel='kt / month')
        ax_top.set_title('EU gasoil/diesel — consumption vs refinery output',
                         loc='left', fontsize=10, fontweight='normal')
        ...
        ax_bot.axhline(mean_gap, color=self.COLORS['REF'], linestyle=':',
                       linewidth=0.8, label=f'Mean gap = {mean_gap:+,.0f} kt/m')
        ax_bot.set(xlabel='Date', ylabel='Consumption − output (kt/m)')
        ax_bot.set_title('Net import requirement — positive = deficit filled by imports',
                         loc='left', fontsize=10, fontweight='normal')
        ax_bot.legend(loc='upper left', fontsize=9)
        self._format_xaxis(ax_bot)

        latest = merged.iloc[-1]
        print(f'EU diesel balance latest ({latest["Date"].date()}): '
        ...
        print(f'Mean monthly gap over sample: {mean_gap:+,.0f} kt ({mean_gap*12/1000:+.1f} Mt/year)')
        print(f'Interpretation: EU structurally needs to import '
              f'~{abs(mean_gap*12/1000):.0f} Mt/year of diesel to cover domestic consumption.')

        fig.suptitle('EU diesel — supply-demand balance',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_eu_jet_consumption(self, MIN_COUNTRIES_REPORTING=10):
        ...
        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        ax.plot(cons_tot['Date'], cons_tot['Value_kt'],
                color=self.COLORS['HIST'], linewidth=self.LW + 0.2,
        ...
        ax.plot(cons_tot['Date'], cons_tot['MA12'],
                color=self.COLORS['REF'], linewidth=self.LW, linestyle='--',
                label='12m MA')
        ...
        print(f'EU jet consumption latest ({latest["Date"].date()}): '
              f'{latest["Value_kt"]:,.0f} kt/m')
        print(f'COVID trough ({covid_trough["Date"].date()}): {covid_trough["Value_kt"]:,.0f} kt/m'
              f' — {100*covid_trough["Value_kt"]/pre_covid_peak:.0f}% of pre-COVID peak')
        print(f'Recovery: latest as % of pre-COVID peak = {100*latest["Value_kt"]/pre_covid_peak:.0f}%')

        fig.suptitle('Jet fuel — demand trajectory',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=1.02)
    ...
    def chart_eu_consumption_annual(self, MIN_COUNTRIES_REPORTING=15):
        ...
        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        bar_colors = [self.COLORS['VOL'] if y >= 2022 else self.COLORS['FLOW']
                      for y in deficit.index]
        bars = ax.bar(deficit.index.astype(str), deficit.values,
                      color=bar_colors, alpha=0.85)
        for bar, val in zip(bars, deficit.values):
            ax.text(bar.get_x() + bar.get_width() / 2, val + 0.4, f'{val:.0f}',
                    ha='center', va='bottom', fontsize=9)
        # Pre-2022 and 2022+ means for context
        pre22 = deficit[deficit.index < 2022].mean()
        post22 = deficit[deficit.index >= 2022].mean()
        ax.axhline(pre22, color=self.COLORS['FLOW'], linestyle=':', linewidth=0.9,
                   label=f'Pre-2022 mean {pre22:.0f} Mt/y')
        ax.axhline(post22, color=self.COLORS['VOL'], linestyle=':', linewidth=0.9,
                   label=f'2022+ mean {post22:.0f} Mt/y')
        ax.set(xlabel='Year', ylabel='Mt / year',
               title='EU diesel deficit — consumption minus refinery output')
        ax.legend(loc='upper left', fontsize=9)
        print('Annual EU diesel deficit (Mt):')
        for y, v in deficit.items():
            print(f'  {y}: {v:+5.1f}')
        print(f'Pre-2022 mean: {pre22:.1f} Mt/y   |   2022+ mean: {post22:.1f} Mt/y   '
              f'step-change: {post22 - pre22:+.1f} Mt/y')

        fig.suptitle('EU diesel deficit — annual view',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
