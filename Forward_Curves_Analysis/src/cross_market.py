...
class CrossMarketAnalysis:
    ...
    def matched_brent_lsgo_spread(self, DATA_ROOT=None, BBL_PER_MT=7.45):
        ...
        root = Path(DATA_ROOT) if DATA_ROOT else (
            Path(cfg.get('PATHS', {}).get('DATA_ROOT', '../Datas')).resolve()
        )
        brent = pd.read_excel(root / 'Raw_datas' / 'Futures' / 'Brent_M1M12.xlsx')
        lsgo  = pd.read_excel(root / 'Raw_datas' / 'Futures' / 'LSGO_M1M12.xlsx')
