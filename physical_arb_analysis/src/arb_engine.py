...
class CargoArb:
    ...
    buy_product: str | None = (
        None  # e.g. 'Brent', 'WTI', 'LSGO', 'ULSD', 'Jet', 'EBOB', 'Dated_Brent'
    )
    buy_month: str | None = (
        None  # e.g. 'M1', 'M2', 'M3' for futures; 'CIF_Med', 'CIF_NWE', 'FOB_ARA' for physical
    )
    sell_product: str | None = None
    sell_month: str | None = None
    ...
    # Volume
    cargo_mt: float | None = None
    capacity_utilization: float = 1.0

    # Freight lookup
    flat_rate: float | None = None
    ws_flat_key: str | None = None
    ...
    # Cost toggles
    financing_days: float | None = None
    demurrage_days: float | None = None
    war_risk_uplift_bps: float = 0.0

    # Metadata (reporting, no effect on math)
    buy_timing: str | None = None
    sell_timing: str | None = None
    date_of_prices: str | None = None
    ...
    def __post_init__(self):
        ...
        if self.vessel not in cfg["VESSEL_SPECS"]:
            raise KeyError(f"Unknown vessel {self.vessel!r}; known: {list(cfg['VESSEL_SPECS'])}")
        self._vessel = cfg["VESSEL_SPECS"][self.vessel]

        # Grade
        if self.grade not in cfg["GRADE_SPECS"]:
            raise KeyError(f"Unknown grade {self.grade!r}; known: {list(cfg['GRADE_SPECS'])}")
        ...
        self.OUTPUT_DIR = PAA_ROOT / cfg["PATHS"]["FIGURES"] if self.save_figures else None
    ...
    def from_preset(cls, preset_key: str, **overrides) -> CargoArb:
        ...
        if preset_key not in cfg["PRESETS"]:
            raise KeyError(f"Preset {preset_key!r} not in config; known: {list(cfg['PRESETS'])}")
    ...
    def financing_usd_bbl(self) -> float:
        rate_ann = (self._analysis["SOFR_BPS"] + self._analysis["TRADER_SPREAD_BPS"]) / 10000.0
    ...
    def _auto_war_zone_uplift_bps(self) -> float:
        ...
        ports = self._cfg["PORTS"]
        regions = {
            ports.get(self.origin_port, {}).get("region"),
            ports.get(self.dest_port, {}).get("region"),
        }
        # Lloyd's listed war zones at snapshot date:
        if "AG" in regions:
            return 100.0  # Hormuz, AG ports
        if "BlackSea" in regions:
            return 75.0  # Russia-Ukraine war zone
        if "WAF" in regions:
            return 25.0  # piracy uplift, low
        return 0.0
    ...
    def breakeven_ws(self) -> float:
        ...
        other = (
            w["port_costs_usd_bbl"]
            + w["canal_tolls_usd_bbl"]
            + w["financing_usd_bbl"]
            + w["insurance_usd_bbl"]
            + w["awrp_usd_bbl"]
    ...
    def sensitivity_ws(self, ws_range: np.ndarray) -> pd.DataFrame:
        return pd.DataFrame(
            [{"ws": w, "net_usd_bbl": self._clone(ws_rate=float(w)).pnl_bbl()} for w in ws_range]
        )

    def sensitivity_grid(self, ws_range: np.ndarray, spread_range: np.ndarray) -> pd.DataFrame:
        rows = []
        for w in ws_range:
    ...
    def _finalize(self, fig, file_key: str | None):
    ...
    def chart_waterfall(self, WS_STRESS_THRESHOLD: float = 180.0):
        ...
        for i, (lbl, val, clr) in enumerate(steps):
            if lbl in ("Gross\nspread", "NET arb"):
                ...
                ax.bar(i, val, bottom=running, color=clr, edgecolor="black", linewidth=0.6)
                ax.annotate(
                    f"{val:+.2f}",
                    xy=(i, running + val),
                    xytext=(0, 6 if val >= 0 else -14),
        ...
        ax.set_title(
            ...
            f"WS {self.ws_rate:.1f} · {status}",
            loc="left",
        )
        return self._finalize(fig, "WATERFALL")

    def chart_sensitivity_ws(self, ws_range: np.ndarray | None = None):
        if ws_range is None:
        ...
        fig, ax = plt.subplots(figsize=self.SIZES["STANDARD"])
        ax.plot(df["ws"], df["net_usd_bbl"], color=self.COLORS["PRICE"], linewidth=self.LW + 0.3)
        ...
        ax.set_title(
            f"{self.origin_port} → {self.dest_port} · {self.vessel} · P&L sensitivity to WS",
            loc="left",
        )
        ax.legend(loc="upper right", fontsize=9)
        return self._finalize(fig, "SENSITIVITY_WS")

    def chart_sensitivity_grid(
        self, ws_range: np.ndarray | None = None, spread_range: np.ndarray | None = None
    ):
        if ws_range is None:
        ...
        im = ax.imshow(
            ...
            vmin=-vmax,
            vmax=vmax,
            extent=[ws_range.min(), ws_range.max(), spread_range.min(), spread_range.max()],
        )
        ax.contour(
            piv.columns,
            piv.index,
    ...
    def chart_route_comparison(cls, arbs: dict):
        ...
        colours = [ref.COLORS["MARGIN"] if r[2] else ref.COLORS["VOL"] for r in rows]
        ax.barh(
            [r[0] for r in rows],
            [r[1] for r in rows],
            color=colours,
        ...
        ax.set_title(
            "Lane comparison — net arb P&L at current WS + forward-price inputs", loc="left"
        )
        return ref._finalize(fig, "ROUTE_COMPARISON")

    @classmethod
    def chart_scenario_backtest(cls, scenarios: pd.DataFrame, ref_arb: CargoArb | None = None):
        ref = (
            ref_arb
            if ref_arb is not None
            else cls.from_preset(
                "USG_NWE_ULSD", ws_rate=100, buy_price=100, sell_price=100, save_figures=True
            )
        )
        colours = [ref.COLORS["MARGIN"] if v >= 0 else ref.COLORS["VOL"] for v in scenarios["net"]]

        fig, (ax_top, ax_bot) = plt.subplots(
            2,
            1,
            figsize=(ref.SIZES["STANDARD"][0], ref.SIZES["STANDARD"][1] + 2),
            gridspec_kw={"height_ratios": [1, 1], "hspace": 0.45},
        )
        ax_top.bar(
            scenarios["event"],
            scenarios["ws"],
            color=ref.COLORS["PRICE"],
            edgecolor="black",
            linewidth=0.6,
        )
        ax_top.set(ylabel="Worldscale (WS)")
        ax_top.set_title(
            f"Freight WS by scenario — {', '.join(scenarios['route'].unique())}",
            loc="left",
            fontsize=10,
        )
        ...
        ax_bot.bar(
            scenarios["event"], scenarios["net"], color=colours, edgecolor="black", linewidth=0.6
        )
        ...
        ax_bot.set_title("Net arb P&L — open (green) / closed (red)", loc="left", fontsize=10)
