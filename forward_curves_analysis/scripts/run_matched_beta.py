...
def report(out, label):
    ...
    print(
        f"Normal (full)         n={len(norm):4d}  mean β = {norm['beta'].mean():+.4f}  median β = {norm['beta'].median():+.4f}"
    )
    print(
        f"Crisis 2022 invasion  n={len(cri_2022):4d}  mean β = {cri_2022['beta'].mean():+.4f}  median β = {cri_2022['beta'].median():+.4f}"
    )
    print(
        f"Crisis 2026 Hormuz    n={len(cri_2026):4d}  mean β = {cri_2026['beta'].mean():+.4f}  median β = {cri_2026['beta'].median():+.4f}"
    )
    print(f"Latest β              {s.iloc[-1]['Date'].date()}  β = {s.iloc[-1]['beta']:+.4f}")
    print(f"Last 30 obs           mean β = {s.tail(30)['beta'].mean():+.4f}")
    return {
        "normal_full": norm["beta"].mean(),
...
out = cma.matched_rolling_hedge_ratio(ANCHOR="LSGO", WINDOW=60, RETURNS="log", PAIRING="M2xM1")
...
print("=" * 60)
print(
    f"Naïve M1×M1   Normal:  {n_summary['normal_full']:+.2f}   Crisis-2026:  {n_summary['crisis_2026']:+.2f}   Latest:  {n_summary['latest']:+.2f}"
)
print(
    f"Matched M2×M1 Normal:  {m_summary['normal_full']:+.2f}   Crisis-2026:  {m_summary['crisis_2026']:+.2f}   Latest:  {m_summary['latest']:+.2f}"
)
print()
print("Trade-block sizing on a 50,000 bbl ULSD-crack-equivalent:")
for label, beta in [
    ("Matched Crisis-2026 mean", m_summary["crisis_2026"]),
...
print()
print("Latest 30 obs β tail:")
print(
    out["series"][["Date", "pair_label", "regime_2state", "beta"]].tail(30).to_string(index=False)
)
