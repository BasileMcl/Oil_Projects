...
NOTEBOOKS_DIR = FCA_ROOT / "notebooks"
...
def events_for_symbol(
    cfg: dict[str, Any], symbol: str, include_observations: bool = False
) -> dict[str, str]:
...
class ForwardCurveAnalysis:
    ...
    def __init__(
        self, COMMODITY: str, CONFIG_PATH: str | Path | None = None, SAVE_FIGURES: bool = True
    ) -> None:
        ...
        if key not in cfg["COMMODITIES"]:
            raise KeyError(
                f"Unknown commodity {COMMODITY!r}. Available: {list(cfg['COMMODITIES'])}"
            )
        ...
        self.RECENT_START = (
            pd.to_datetime(ana["RECENT_START_DATE"]) if ana.get("RECENT_START_DATE") else None
        )
        ...
        self.OUTPUT_DIR = (
            self.ROOT / cfg["PATHS"]["FIGURES"] / com["SUBDIR"] if SAVE_FIGURES else None
        )
    ...
    def load(self) -> ForwardCurveAnalysis:
        ...
        if suffix == ".csv":
            ...
            def date_parse(s):
                return pd.to_datetime(s, format=self.DATE_FORMAT)
        ...
        self.df_raw = df
        msg = (
            f"[{self.COMMODITY}] Loaded {len(df)} rows | "
    ...
    def build(self) -> ForwardCurveAnalysis:
        ...
        roll_tag = (
            f"{n_rolls} rolls detected"
            if not self.PRE_ROLLED
            else "pre-rolled ICE series (individual rolls not recoverable)"
        )
        msg = f"[{self.COMMODITY}] Pipeline complete — {len(self.df_rolling)} rows, {roll_tag}"
        logger.info(msg)
        print(msg)
        return self
    ...
    def summary_stats(self):
        ...
        stats = {
            ...
            f"Days |Z|>{self.Z_EXTREME:.0f} (vs {self.Z_EXPECTED_PCT}% expected)": f"{z_hits} ({100 * z_hits / len(df):.1f}%)",
        }
    ...
    def calibrate_thresholds(self, IS_END="2024-12-31", QUANTILES=None, SPREAD="M1_M6"):
        ...
        return {
            ...
            "is_window": (
                is_df["Date"].min().date().isoformat(),
                is_df["Date"].max().date().isoformat(),
                len(is_df),
            ),
            "oos_window": (
                (
                    oos_df["Date"].min().date().isoformat(),
    ...
    def _two_panel_chart(self, draw_panel, df, title: str, file_key: str, extra_height: int = 4):
    ...
    def chart_forward_curves(self, SNAPSHOT_DATES=None):
        ...
        for i, d in enumerate(dates):
            ...
            ax.plot(xs, curve, marker="o", linewidth=2, color=cycle[i % len(cycle)], label=d)
    ...
    def chart_short_structure(self):
        return self._spread_chart(
            ["M1_M2", "M1_M3"], "Short-Term Structure: M1-M2 and M1-M3", "SHORT_STRUCTURE"
        )
    ...
    def chart_zscore(self, SPREAD: str = "M1_M3"):
        ...
        subtitle = f"({self.Z_WINDOW}-day rolling)" if mode_tag == "rolling" else "(IS-anchored)"
    ...
    def _zscore_panel(self, ax, df, spread, subtitle, CLIP=5.0):
        ...
        ax.plot(
            ...
            linewidth=self.LW,
            label=f"Z {spread.replace('_', '-')}",
        )
        for lvl, ls in zip(self.Z_LINES["LEVELS"], self.Z_LINES["LINESTYLES"], strict=False):
            ax.axhline(
                lvl, color=self.COLORS["REF"], linestyle=ls, linewidth=0.8, alpha=self.A["REF"]
            )
        ax.set(
            xlabel="Date",
    ...
    def chart_distribution(self):
        ...
        fig.suptitle(
            f"{self.COMMODITY} M1-M3 Distribution — IS vs OOS",
            x=0.01,
            ha="left",
            fontsize=13,
            fontweight="bold",
    ...
    def chart_event_timeline(self, EVENTS=None):
        ...
        def _panel(ax, frame, panel_events, subtitle):
            ax.plot(
                frame["Date"], frame["M1_M3"], color=c["M1_M3"], linewidth=self.LW, label="M1-M3"
            )
            self._color_axis(ax, "y", c["M1_M3"], f"M1-M3 ({self.UNIT})")
            ax2 = ax.twinx()
            ax2.plot(
                frame["Date"],
                frame[self.VOL_COL] * 100,
                color=c["VOL"],
                linewidth=self.LW,
                alpha=self.A["SECONDARY"],
            )
            ...
            for j, (d, label) in enumerate(panel_events.items()):
                ...
                ax.annotate(
                    ...
                    fontsize=8,
                    bbox=dict(
                        boxstyle="round,pad=0.25", facecolor="white", edgecolor=c["REF"], alpha=0.9
                    ),
        ...
        fig.suptitle(
            f"{self.COMMODITY} — Event Timeline · M1-M3 spread & volatility",
            x=0.01,
            ha="left",
            fontsize=12,
            fontweight="bold",
    ...
    def chart_dashboard(self):
        ...
        for lvl, ls in zip(self.Z_LINES["LEVELS"], self.Z_LINES["LINESTYLES"], strict=False):
            axes[1, 0].axhline(
                lvl, color=c["REF"], linestyle=ls, linewidth=0.8, alpha=self.A["REF"]
            )
        ...
        axes[1, 1].set(
            title=f"{self.VOL_WINDOW}-Day Annualised Volatility (%)", ylabel="Volatility (%)"
        )
        axes[1, 1].legend(loc="upper left", fontsize=9)

        for ax in axes.flat:
            self._format_xaxis(ax)
        fig.suptitle(f"{self.COMMODITY} Market Dashboard", fontsize=14, fontweight="bold", y=1.002)
    ...
    def chart_dated_event_study(self, EVENT_DATE="2026-04-07"):
        ...
        df = self.df_rolling
        window = df[(df["Date"] >= dated["Date"].min()) & (df["Date"] <= dated["Date"].max())]
        ...
        ax_top.plot(
            ...
            color=self.COLORS["VOL"],
            linewidth=self.LW,
            label="Dated Brent (DTD)",
        )
        if EVENT_DATE:
            ax_top.axvline(
                pd.to_datetime(EVENT_DATE), color="black", linestyle="--", linewidth=1, alpha=0.6
            )
            ax_top.annotate(
                f"Event: {EVENT_DATE}",
                xy=(pd.to_datetime(EVENT_DATE), dated["Dated"].max()),
                xytext=(10, -5),
                textcoords="offset points",
                fontsize=9,
            )
        ...
        ax_top.set_title(
            "Physical Dated Brent vs paper front-month",
            loc="left",
            fontsize=10,
            fontweight="normal",
        )
        ...
        ax_bot.plot(merged["Date"], basis, color=self.COLORS["M1_M3"], linewidth=self.LW)
        self._zero_line(ax_bot)
        ax_bot.set(ylabel=f"Basis Dated − M1 ({self.UNIT})", xlabel="Date")
        ax_bot.set_title(
            "Physical-paper basis (positive = physical premium)",
            loc="left",
            fontsize=10,
            fontweight="normal",
        )
        ...
        fig.suptitle(
            f"{self.COMMODITY} — Dated Brent event study",
            x=0.01,
            ha="left",
            fontsize=13,
            fontweight="bold",
    ...
    def chart_long_history_context(self, T74_PATH=None, ANNOTATE_EVENTS=True):
        ...
        brent_m1 = t74[
            (t74["instrument"] == "ICE Brent") & (t74["contract"] == "1st forward month")
        ].sort_values("date")
        wti_m1 = t74[
            (t74["instrument"] == "NYMEX WTI") & (t74["contract"] == "1st forward month")
        ].sort_values("date")

        fig, ax = plt.subplots(figsize=self.SIZES["STANDARD"])
        ax.plot(
            brent_m1["date"],
            brent_m1["price_usd_bbl"],
            color=self.COLORS["PRICE"],
            linewidth=self.LW,
        ...
        ax.axvspan(
            is_end, oos_end, color="#888888", alpha=0.15, label=f"OOS window ({is_end.year + 1}+)"
        )
        ...
        ax.axhline(
            latest["price_usd_bbl"],
            color=self.COLORS["M1_M3"],
            linestyle=":",
            linewidth=0.9,
            label=f"Latest Brent {latest['price_usd_bbl']:.0f} USD/bbl ({pct:.0f}th pct of 36y)",
        )
        ...
        ax.set(
            xlabel="Date",
            ylabel=f"Front-month ({self.UNIT})",
        ...
        print("Brent monthly — 36y context")
        ...
        print(
            f"   Latest : {latest['price_usd_bbl']:.1f} on {latest['date'].strftime('%Y-%m')} "
    ...
    def chart_hmm_regime(self, SPREAD="M1_M6", N_STATES=2, FIT_ON="full"):
        ...
        ax.plot(
            hmm_df["Date"],
            hmm_df[SPREAD],
            color=self.COLORS["M1_M6"],
            linewidth=self.LW,
            label=f"{SPREAD.replace('_', '-')} ({self.UNIT})",
        )
        self._color_axis(ax, "y", self.COLORS["M1_M6"], f"{SPREAD.replace('_', '-')} ({self.UNIT})")
        ax2 = ax.twinx()
        ax2.plot(
            hmm_df["Date"],
            hmm_df[crisis_col],
        ...
        fig.suptitle(
            f"{self.COMMODITY} HMM regime on {SPREAD.replace('_', '-')}",
            x=0.01,
            ha="left",
            fontsize=13,
            fontweight="bold",
        ...
        crisis_days_oos = ((hmm_df["Date"] > self.Z_IS_END) & (hmm_df[crisis_col] > 0.5)).sum()
        oos_n = (hmm_df["Date"] > self.Z_IS_END).sum()
        is_crisis_days = ((hmm_df["Date"] <= self.Z_IS_END) & (hmm_df[crisis_col] > 0.5)).sum()
        is_n = (hmm_df["Date"] <= self.Z_IS_END).sum()
        print(
            f"IS  P(crisis)>0.5 days: {is_crisis_days}/{is_n} ({100 * is_crisis_days / is_n:.1f}%)"
        )
        print(
            f"OOS P(crisis)>0.5 days: {crisis_days_oos}/{oos_n} ({100 * crisis_days_oos / oos_n:.1f}%)"
        )
    ...
    def _spread_panel(self, ax, df, columns, subtitle, YSCALE="linear", SYMLOG_THRESHOLD=1.0):
    ...
    def _build_contracts(self):
        ...
        for idx in df.index:
            ...
            available = [
                c for c in self.contract_cols if pd.notna(df.loc[idx, c]) and c not in excl
            ]
    ...
    def _add_volatility(self):
        ...
        df[self.VOL_COL] = df["M1_return"].rolling(self.VOL_WINDOW).std() * np.sqrt(
            self.ANNUAL_DAYS
        )
