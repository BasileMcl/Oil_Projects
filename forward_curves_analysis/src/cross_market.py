...
class CrossMarketAnalysis:
    ...
    def __init__(self, COMMODITIES, CONFIG_PATH=None, SAVE_FIGURES=True):
        ...
        self.ROOT = (
            ...
            if CONFIG_PATH
            else Path(ForwardCurveAnalysis.__module__).resolve().parent.parent
        )
        ...
        self.OUTPUT_DIR = (
            Path(cfg["PATHS"]["FIGURES"]).resolve() / "cross_market" if SAVE_FIGURES else None
        )
        ...
        if SAVE_FIGURES and not Path(cfg["PATHS"]["FIGURES"]).is_absolute():
            ...
            self.OUTPUT_DIR = (
                Path(DEFAULT_CONFIG).resolve().parent / cfg["PATHS"]["FIGURES"] / "cross_market"
            )
    ...
    def merge(self):
        ...
        print(
            f"[CrossMarket] merged: {len(self.merged)} common days "
    ...
    def correlations(self):
        ...
        for i, a in enumerate(self.keys):
            for b in self.keys[i + 1 :]:
                for col in ("M1", "M1_M3", "M1_M6", "Vol"):
                    ...
                    logret = (
                        (np.log(m[c1]) - np.log(m[c1].shift(1))).corr(
                            np.log(m[c2]) - np.log(m[c2].shift(1))
                        )
                        if col == "M1"
                        else np.nan
                    )
        ...
        with pd.option_context(
            "display.max_columns", None, "display.width", 220, "display.expand_frame_repr", False
        ):
    ...
    def annual_coupling(self, SPREAD="M1_M3", A=None, B=None):
        ...
        g = m.groupby("year").apply(lambda d: d["dA"].corr(d["dB"])).rename("r").reset_index()
        return g
    ...
    def matched_brent_lsgo_spread(self, DATA_ROOT=None, BBL_PER_MT=7.45):
        ...
        root = (
            Path(DATA_ROOT)
            if DATA_ROOT
            else (Path(cfg.get("PATHS", {}).get("DATA_ROOT", "../data")).resolve())
        )
        ...
        merged = (
            brent.merge(lsgo, on="Date", how="inner").sort_values("Date").reset_index(drop=True)
        )
    ...
    def rolling_hedge_ratio(self, Y, X, WINDOW=90, SPREAD="M1_M3"):
        ...
        m["dY"] = m[f"{y_key}_{SPREAD}"].diff()
        m["dX"] = m[f"{x_key}_{SPREAD}"].diff()
    ...
    def matched_rolling_hedge_ratio(
        self,
        ANCHOR="LSGO",
        WINDOW=60,
        BBL_PER_MT=7.45,
        ...
        if PAIRING == "M1xM1":
            ...
            spread_df = (
                ...
                .dropna(subset=["brent_m1_bbl", "crack_matched_bbl"])
        ...
        by_regime = (
            valid.groupby("regime_2state")["beta"]
    ...
    def hedge_in_dollars(
        self, Y, X, POSITIONS_MT=(50_000, 100_000, 500_000), WINDOW=90, SPREAD="M1_M3"
    ):
        ...
        print(
            f"  {'Y position (MT)':>20s}  {'Current β (bbls)':>22s}  "
        ...
        print()
        print(
            f"Ratio current / historical: {latest / hist:.1f}× — static hedges mis-size by this factor."
        )
    ...
    def chart_regime_conditional_correlation(self, ANCHOR_COMMODITY, METRIC="M1_M3"):
        ...
        im = ax.imshow(corr.values.astype(float), vmin=0, vmax=1, cmap="YlOrRd", aspect="auto")
        ax.set_xticks(range(len(corr.columns)))
        ax.set_xticklabels(corr.columns, rotation=0)
        ax.set_yticks(range(len(corr.index)))
        ax.set_yticklabels([f"{r} (n={ns.loc[r]})" for r in corr.index])
        for i, _r in enumerate(corr.index):
            for j, _c in enumerate(corr.columns):
                v = corr.iloc[i, j]
                if pd.notna(v):
                    ax.text(
                        j,
        ...
        if self.OUTPUT_DIR:
            ...
            fig.savefig(self.OUTPUT_DIR / "cross_08_regime_conditional_corr.png", **self.SAVE_OPTS)
        return fig
    ...
    def chart_scatter(self):
        ...
        for ax, (xc, yc, title) in zip(axes.flat, pairs, strict=False):
            r = m[xc].corr(m[yc])
            ax.scatter(
                m[xc],
                m[yc],
    ...
    def chart_lead_lag(self, LAGS=range(-10, 11)):
        ...
        ax.legend()
        max_p = max(zip(lags, cp, strict=False), key=lambda t: abs(t[1]))
        max_s = max(zip(lags, cs, strict=False), key=lambda t: abs(t[1]))
    ...
    def chart_annual_coupling(self, SPREAD="M1_M3"):
        ...
        bars = ax.bar(
            g["year"].astype(str), g["r"], color=self.COLORS["M1_M3"], edgecolor="white", alpha=0.85
        )
        ax.axhline(0, color="black", linewidth=0.8, alpha=0.5)
        for bar, r in zip(bars, g["r"], strict=False):
            if pd.isna(r):
                continue
            ax.text(
        ...
        ax.set(
            xlabel="Year",
            ylabel=f"Δ{SPREAD.replace('_', '-')} correlation ({a} vs {b})",
            ylim=(-0.2, 1.0),
        )
    ...
    def chart_crack_spread(self, NUMERATOR=None, DENOMINATOR=None, BBL_PER_MT=None):
        ...
        ax_top.plot(
            ...
            color=self.COLORS["PRICE"],
            linewidth=self.LW,
            label=f"{d} M1 (USD/bbl)",
        )
        ax_top.set(ylabel="USD/bbl")
        ax_top.set_title(
            f"{n} vs {d} — unit-aligned front-month", loc="left", fontsize=10, fontweight="normal"
        )
        ...
        ax_bot.set_title(
            "Crack spread — refining-margin proxy", loc="left", fontsize=10, fontweight="normal"
        )
        ...
        fig.suptitle(
            f"{n} − {d} crack spread", x=0.01, ha="left", fontsize=13, fontweight="bold", y=0.995
        )
    ...
    def chart_rolling_hedge_ratio(self, Y=None, X=None, WINDOW=90, SPREAD="M1_M3"):
        ...
        ax.set_title(f"Rolling hedge-ratio {y} on {x} ({SPREAD.replace('_', '-')})", loc="left")
        ax.legend(loc="upper left")
        self._format_xaxis(ax)

        last = m["beta"].dropna().iloc[-1] if m["beta"].dropna().size else np.nan
        print(
            f"Latest rolling β : {last:+.3f}   (positive = {y} spread widens when {x} spread widens)"
        )
    ...
    def chart_basin_spread(self, Y="BRENT", X="WTI", ANNOTATE_EVENTS=True):
        ...
        ax.plot(m["Date"], m["spread"], color=self.COLORS["M1_M3"], linewidth=self.LW)
        ...
        if ANNOTATE_EVENTS:
            ...
            for dt, lbl in [
                (pd.Timestamp("2020-12-03"), "OPEC+ rebalance + vaccine"),
                (pd.Timestamp("2022-03-01"), "Russia invasion"),
                (pd.Timestamp("2026-04-13"), "Hormuz blockade"),
            ]:
                if m["Date"].min() <= dt <= m["Date"].max():
                    ax.axvline(dt, color="black", linestyle="--", linewidth=0.8, alpha=0.55)
                    y_top = m["spread"].max()
                    ax.annotate(
                        lbl,
                        xy=(dt, y_top),
        ...
        ax.set_title(f"{y} − {x} M1 spread — trans-Atlantic basin", loc="left")
        ax.legend(loc="upper left")
    ...
    def chart_correlation_matrix(self, METRIC="M1_M3", DIFFERENCED=True):
        ...
        ax.set_xticklabels(self.keys, rotation=0)
