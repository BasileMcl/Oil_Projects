...
src/
  ...
  futures_calendar.py          contract-month alignment for matched-β work
scripts/
  run_matched_beta.py          regime-conditional ULSD-Brent β pipeline
  regime_update.py             3-line weekly regime header + state log
config.yaml                    per-commodity config + regime thresholds
requirements.txt               inherits ../requirements-base.txt + hmmlearn
generate_reports.sh            runs all 4 notebooks, exports HTML

notebooks/
  01_brent.ipynb               Brent M1-M12, ICE rolling front-month
  02_lsgo.ipynb                LSGO M1-M12, EU diesel proxy
  03_wti.ipynb                 WTI M1-M12, US-inland insulation
  04_cross_market.ipynb        n-way correlation, rolling hedge β
...
Portfolio-wide styling comes from `../plot_config.yaml`, event timeline from `../major_dates.yaml`. `config.yaml` holds only project-specific content (paths, thresholds, snapshot dates, chart filenames).

### Notebook scope

- **01_brent.ipynb** drives `ForwardCurveAnalysis(COMMODITY='BRENT')`. Front-month M1-M12 from 2019-04-16. Regime thresholds 1.07 / 1.33 / 1.51 USD/bbl on M1-M6. Dated Brent overlay (2026-01-02 to 2026-04-14, 69 days) supports the Dated-vs-paper event study.
- **02_lsgo.ipynb** drives `ForwardCurveAnalysis(COMMODITY='LSGO')`. M1-M12 from 2019-12-13. Regime thresholds 3.60 / 6.71 / 8.25 USD/MT on M1-M6, CDF-calibrated on 2019-12 to 2024-12.
- **03_wti.ipynb** drives `ForwardCurveAnalysis(COMMODITY='WTI')`. Longest sample, from 2018-10-03. Regime thresholds 1.15 / 1.44 / 1.61 USD/bbl on M1-M6. Note: ICE's rolling front-month rolls before expiry, so the 2020-04-20 intraday minus-$37 print is not in the series (M1 min = $41.21).
- **04_cross_market.ipynb** drives `CrossMarketAnalysis`. Correlation matrices (levels vs first-difference), rolling correlation, lead-lag, simple crack, rolling hedge β, Brent-WTI trans-Atlantic spread.
...
- **Crack overlay as a diagnostic** in LSGO notebook (one cell reading `../data/csv/crack_spreads_clean.csv` and plotting it against LSGO M1-M3).
