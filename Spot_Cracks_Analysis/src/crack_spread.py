...
class CrackSpreadAnalysis(Chartable):
    ...
    def load(self):
        ...
        self.df_raw = spot.merge(bfut, on='Date', how='outer').sort_values('Date').reset_index(drop=True)
    ...
    def build(self, SLATE='3-2-1', REGION='NWE', BRENT_SOURCE='M1'):
        ...
        df['margin'] = (w['EBOB']  * df['EBOB_bbl']
                      + w['ULSD']  * df['ULSD_bbl']
                      + w['FO']    * df['FO_bbl']
                      - w['BRENT'] * df['Brent_M1_bbl']) / n_crude
        ...
        print(f'[CrackSpread {SLATE}/{REGION} · crude={BRENT_SOURCE}] {len(df)} days  '
    ...
    def summary_stats(self):
        ...
        out = pd.DataFrame.from_dict(stats, orient='index',
                                     columns=[f'{self.slate} / {self.region}'])
        print(out.to_string())
        return out
    ...
    def calibrate_regime_thresholds(self):
        ...
        def classify(x):
            ...
            if x < thr['CRISIS_LO']:  return 'Crisis-Low'
            if x < thr['STRESS_LO']:  return 'Stress-Low'
            if x > thr['CRISIS_HI']:  return 'Crisis-High'
            if x > thr['STRESS_HI']:  return 'Stress-High'
            return 'Normal'
    ...
    def chart_time_series(self, OVERLAY_EVENTS=True):
        ...
        for ax, frame, subtitle, overlay in [(ax_full, df, 'Full sample', False),
                                             (ax_recent, self._recent(df),
                                              f'Last {self.RECENT_MONTHS} months',
                                              OVERLAY_EVENTS)]:
            ax.plot(frame['Date'], frame['margin'], color=self.COLORS['CRACK'],
                    linewidth=self.LW, label=f'{self.slate} / {self.region}')
            ax.axhline(df['margin'].mean(), color=self.COLORS['REF'],
                       linestyle=':', linewidth=0.8, alpha=0.6,
                       label=f'Full-sample mean = {df["margin"].mean():.2f}')
            self._zero_line(ax)
            ax.set(xlabel='Date', ylabel=f'Margin (USD/bbl)')
            ax.set_title(subtitle, loc='left', fontsize=10, fontweight='normal')
            ax.legend(loc='upper left', fontsize=9)
            self._format_xaxis(ax)
            if overlay:
                self._add_event_markers(ax)
        fig.suptitle(f'NWE {self.slate} refining margin — {self.region}',
        ...
        oos_mean = df.loc[df['is_oos']=='OOS', 'margin'].mean() if (df['is_oos']=='OOS').any() else float('nan')
        print(f'Read: {self.slate}/{self.region} margin is {last["margin"]:.1f} USD/bbl on {last["Date"].date()} '
    ...
    def chart_three_brent_tracks(self):
        ...
        ax.plot(df['Date'], df['Brent_M1_bbl'],
        ...
        if 'Brent_spot_bbl' in df.columns and df['Brent_spot_bbl'].notna().any():
            ...
            ax.plot(sp['Date'], sp['Brent_spot_bbl'],
                    color=self.COLORS['HIST'], linewidth=self.LW,
                    alpha=0.85, label='Brent spot (Platts assessed)')
        if 'Dated_Brent_bbl' in df.columns and df['Dated_Brent_bbl'].notna().any():
            dt = df.dropna(subset=['Dated_Brent_bbl'])
            ax.plot(dt['Date'], dt['Dated_Brent_bbl'],
                    color=self.COLORS['VOL'], linewidth=self.LW + 0.4,
        ...
        if 'Dated_Brent_bbl' in df.columns:
            ...
            if len(ovl):
                ...
                print(f'Dated Brent vs M1 futures: {len(ovl)} overlap days  '
                      f'(only {ovl["Date"].min().date()} to {ovl["Date"].max().date()})')
                print(f'  mean basis = {basis.mean():+.2f} USD/bbl  '
        ...
        if 'Brent_spot_bbl' in df.columns:
            ...
            if len(ovl2):
                ...
                print(f'Brent spot vs M1 futures: {len(ovl2)} overlap days  '
                      f'mean basis = {basis2.mean():+.2f}  | max = {basis2.max():+.2f}')

        fig.suptitle('Spot, futures, Dated — three different Brent prices',
    ...
    def chart_distribution(self):
        ...
        for ax, xlim, title in [(ax_full, None, 'Full range'),
                                (ax_zoom,
                                 (is_.quantile(0.005), is_.quantile(0.995))
                                 if len(is_) > 20 else None,
                                 'IS-range zoom')]:
            ax.hist(is_,  bins=self.HIST_BINS, color=self.COLORS['PRICE'],
                    alpha=0.55, density=True, edgecolor='white',
                    label=f'IS (n={len(is_)})')
            if len(oos):
                ax.hist(oos, bins=self.HIST_BINS, color=self.COLORS['HIST'],
                        alpha=0.55, density=True, edgecolor='white',
                        label=f'OOS (n={len(oos)})')
            ax.axvline(is_.mean(),  color=self.COLORS['PRICE'], linestyle='--',
                       linewidth=1.5, alpha=0.8, label=f'IS μ = {is_.mean():.2f}')
            if len(oos):
                ax.axvline(oos.mean(), color=self.COLORS['VOL'], linestyle='--',
                           linewidth=1.5, alpha=0.8, label=f'OOS μ = {oos.mean():.2f}')
            if xlim is not None:
                ax.set_xlim(xlim)
            ax.set(xlabel=f'Margin (USD/bbl)', ylabel='Density')
            ax.set_title(title, loc='left', fontsize=10, fontweight='normal')
            ax.legend(fontsize=8)
            ax.grid(True, alpha=0.3, axis='y'); ax.grid(False, axis='x')
        fig.suptitle(f'NWE {self.slate}/{self.region} — IS vs OOS distribution',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=1.02)
    ...
    def chart_components(self):
        df = self.df
        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        ax.plot(df['Date'], df['ULSD_bbl'],  color=self.COLORS['CRACK'],
                linewidth=self.LW, label=f'ULSD {self.region} (USD/bbl)')
        ax.plot(df['Date'], df['EBOB_bbl'],  color=self.COLORS['MARGIN'],
                linewidth=self.LW, label='Gasoline EBOB (USD/bbl)')
        ax.plot(df['Date'], df['Brent_M1_bbl'], color=self.COLORS['PRICE'],
                linewidth=self.LW, label='Brent M1 (USD/bbl)')
        ax.set(xlabel='Date', ylabel='USD/bbl')
    ...
    def chart_regime(self):
        ...
        for regime, color in palette.items():
            ...
            if mask.any():
                ax.scatter(df.loc[mask, 'Date'], df.loc[mask, 'margin'],
                           c=color, label=regime, s=22, alpha=self.A['REGIME'])
        for k, v in thr.items():
            ax.axhline(v, color=self.COLORS['REF'], linestyle='--',
                       linewidth=0.8, alpha=0.5)
        ax.set(xlabel='Date', ylabel=f'{self.slate} margin (USD/bbl)')
        ax.set_title(f'NWE {self.slate}/{self.region} — regime classification',
                     loc='left')
    ...
    def chart_med_vs_nwe(self):
        ...
        self.build(SLATE='ULSD', REGION='NWE'); nwe = self.df[['Date', 'margin']].rename(columns={'margin':'NWE'})
        self.build(SLATE='ULSD', REGION='MED'); med = self.df[['Date', 'margin']].rename(columns={'margin':'MED'})
        ...
        ax_top.plot(merged['Date'], merged['NWE'], color=self.COLORS['CRACK'],
                    linewidth=self.LW, label='ULSD NWE crack')
        ax_top.plot(merged['Date'], merged['MED'], color=self.COLORS['MARGIN'],
                    linewidth=self.LW, label='ULSD Med crack')
        ...
        ax_bot.plot(merged['Date'], merged['diff'], color=self.COLORS['PRICE'],
                    linewidth=self.LW)
        self._zero_line(ax_bot)
        ax_bot.set(xlabel='Date', ylabel='NWE − Med (USD/bbl)')
        ax_bot.set_title('Regional differential — positive = NWE premium',
        ...
        fig.suptitle('ULSD refining margin — NWE vs Med',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
        # Restore
    ...
    def chart_all_slates(self):
        ...
        for i, slate in enumerate(self.SLATES):
            ...
            ax.plot(self.df['Date'], self.df['margin'],
                    color=palette[i % len(palette)], linewidth=self.LW,
                    alpha=self.A['PRIMARY'], label=slate)
        self._zero_line(ax)
    ...
    def chart_seasonal(self, SLATE=None):
        ...
        fig, ax = plt.subplots(figsize=self.SIZES['WIDE'])
        ax.bar(seasonal['month'], seasonal['mean'], yerr=seasonal['std'],
               color=self.COLORS['MARGIN'], alpha=0.85, edgecolor='white',
               capsize=4)
        ax.axhline(df['margin'].mean(), color=self.COLORS['REF'],
        ...
        ax.set_title(f'Seasonality — {slate}/{self.region} (IS window only)',
                     loc='left')
        ax.legend(loc='upper left')
    ...
    def chart_executive_dashboard(self, FA=None, RA=None):
        ...
        ax = axes[0, 0]
        ax.plot(self.df['Date'], self.df['margin'], color=self.COLORS['CRACK'],
                linewidth=1.2)
        ax.axhline(self.df['margin'].mean(), color=self.COLORS['REF'],
                   linestyle='--', linewidth=0.7)
        ax.set_title(f'NWE {self.slate} refining margin  ·  '
                     f'mean {self.df["margin"].mean():.1f} USD/bbl',
                     loc='left', fontsize=10)
        ...
        if FA is not None:
            ...
            ax.plot(spr['Date'], spr['SPR_Crude_Stocks_Mbbl'],
                    color=FA.COLORS['SPR'], linewidth=1.3)
            ax.set_title(f'SPR crude stocks  ·  '
                         f'latest {spr.iloc[-1]["SPR_Crude_Stocks_Mbbl"]:.0f} Mbbl',
                         loc='left', fontsize=10)
            ax.set_ylabel('Mbbl')
            FA._format_xaxis(ax)
        ...
        if FA is not None:
            ...
            ax.plot(util['Date'], util['Refinery_Utilization_Pct'],
                    color=FA.COLORS['PRICE'], linewidth=1.2,
                    marker='o', markersize=3)
            ax.set_title(f'US refinery utilisation  ·  '
        ...
        if RA is not None:
            ax = axes[1, 1]
            c = RA.cracks[['Date', 'Brent_ULSD_NWE_Crack',
                            'Brent_EBOB_Crack']].dropna()
            c['prem'] = c['Brent_ULSD_NWE_Crack'] - c['Brent_EBOB_Crack']
            ax.plot(c['Date'], c['prem'], color=RA.COLORS['MARGIN'],
                    linewidth=1.2)
            ax.axhline(c['prem'].mean(), color=RA.COLORS['REF'],
                       linestyle='--', linewidth=0.7)
            ax.set_title(f'Diesel − gasoline crack premium  ·  '
                         f'mean {c["prem"].mean():+.1f} USD/bbl',
                         loc='left', fontsize=10)
            ax.set_ylabel('USD/bbl')
            RA._format_xaxis(ax)

        fig.suptitle('Market dashboard — 45-second read',
    ...
    def pnl_scaling(self, REFINERY_KBPD=100, UTILISATION=0.90, PRINT=True):
        ...
        if PRINT:
            print(f'Volumetric P&L — {REFINERY_KBPD} kbpd refinery @ {UTILISATION*100:.0f}% util = {kbd:.0f} kbd throughput')
            print(f'{self.slate}/{self.region} crack')
            print(f'   Latest ({self.df.iloc[-1]["Date"].date()}) : {latest_crack:>6.2f} USD/bbl  →  '
                  f'{daily_latest/1e6:>6.2f} MUSD/day  ({annual_latest:>6.0f} MUSD/year)')
            print(f'   Full-sample mean             : {mean_crack:>6.2f} USD/bbl  →  '
                  f'{daily_mean/1e6:>6.2f} MUSD/day  ({annual_mean:>6.0f} MUSD/year)')
            print(f'   95th pct (good-margin day)   : {p95_crack:>6.2f} USD/bbl  →  '
                  f'{kbd*1000*p95_crack/1e6:>6.2f} MUSD/day')
            print(f'   5th pct (bad-margin day)     : {p05_crack:>6.2f} USD/bbl  →  '
                  f'{kbd*1000*p05_crack/1e6:>6.2f} MUSD/day')
            print(f'Reality check: a mid-sized refinery (≈100 kbpd) at mean margin earns ≈ '
                  f'${annual_mean:.0f} M/year gross — before opex (~$3-4/bbl) and hedging.')
        return {'latest_crack': latest_crack,
    ...
    def chart_rolling_hedge_ratio(self, WINDOW=60):
        ...
        se_beta = (df['resid'].rolling(WINDOW).std()
                   / (var * WINDOW).pow(0.5))
        df['beta_lo'] = df['beta'] - 1.96 * se_beta
        df['beta_hi'] = df['beta'] + 1.96 * se_beta

        fig, ax = plt.subplots(figsize=self.SIZES['STANDARD'])
        ax.fill_between(df['Date'], df['beta_lo'], df['beta_hi'],
                        color=self.COLORS['PRICE'], alpha=0.20,
                        label='95% CI')
        ax.plot(df['Date'], df['beta'], color=self.COLORS['PRICE'],
                linewidth=self.LW + 0.3, label=f'Rolling {WINDOW}-day β')
        mean_beta = df['beta'].mean()
        ax.axhline(mean_beta, color=self.COLORS['REF'], linestyle=':',
                   linewidth=0.8, label=f'Mean β = {mean_beta:+.3f}')
        self._zero_line(ax)
        ax.set(xlabel='Date',
               ylabel=f'β (Δ{self.slate} crack vs ΔBrent)')
        ax.set_title(f'Rolling hedge ratio — {self.slate}/{self.region} crack vs Brent M1',
                     loc='left')
        ax.legend(loc='upper left', fontsize=9)
        self._format_xaxis(ax)
        self._add_event_markers(ax)
        ...
        latest = df.iloc[-1]
        print(f'Rolling β ({WINDOW}d): latest {latest["beta"]:+.3f}  '
        ...
        if latest['beta'] * mean_beta > 0 and abs(latest['beta'] / mean_beta) > 2:
            print(f'⚠ current β is {latest["beta"]/mean_beta:.1f}× the mean — static hedges mis-size')
        return self._finalize(fig, 'CRACK_ROLL_HEDGE')

    def chart_rolling_hedge_ratio_by_regime(self, WINDOW=60,
                                             ANCHOR_COMMODITY='LSGO'):
        ...
        p1_path = (_Path(__file__).resolve().parent.parent.parent
                   / 'Forward_Curves_Analysis' / 'src'
        ...
        fca = ForwardCurveAnalysis(COMMODITY=ANCHOR_COMMODITY,
                                    CONFIG_PATH=str(p1 / 'config.yaml'),
                                    SAVE_FIGURES=False).load().build()
        fca.calibrate_thresholds(IS_END='2024-12-31')
        regime = fca.df_rolling[['Date', 'Regime']].rename(
            columns={'Regime': 'lsgo_regime'})
        ...
        for regime_name, sub in df.groupby('lsgo_regime'):
            ...
            clr = regime_colors.get(regime_name,
                   regime_colors.get(regime_name.replace('-', '_'), '#888'))
            # Some regimes are like 'Crisis-High', fall back to grey
            if isinstance(clr, dict):
                clr = '#888'
            ax.scatter(sub['Date'], sub['beta'], color=clr, s=10, alpha=0.7,
                       label=f'{regime_name} (n={len(sub.dropna(subset=["beta"]))}, '
                             f'μ={sub["beta"].mean():+.2f})')

        mean_beta = df['beta'].mean()
        ax.axhline(mean_beta, color=self.COLORS['REF'], linestyle=':',
                   linewidth=0.8, label=f'Overall mean β = {mean_beta:+.2f}')
        self._zero_line(ax)
        ax.set(xlabel='Date',
               ylabel=f'β (Δ{self.slate} crack vs ΔBrent)')
        ax.set_title(
        ...
        if crisis_mask.any() and normal_mask.any():
            ...
            print(f'Crisis/Normal ratio = {ratio:.2f}× — '
    ...
    def chart_hmm_regime(self, N_STATES=2):
        ...
        if bic_by_k:
            ...
            print(f'  → Lowest BIC at K={best_k}; adding states over-fits on '
        ...
        fig, (ax_ts, ax_st) = plt.subplots(
            2, 1, figsize=(self.SIZES['STANDARD'][0], self.SIZES['STANDARD'][1] + 3),
            sharex=True, gridspec_kw={'height_ratios': [2, 1], 'hspace': 0.2})

        palette = [self.COLORS['MARGIN'], self.COLORS['CRACK'], self.COLORS['VOL']]
        ax_ts.plot(df['Date'], df['margin'], color=self.COLORS['PRICE'],
                   linewidth=self.LW, alpha=0.8)
        for s in range(N_STATES):
            mask = df['state'] == s
            ax_ts.scatter(df.loc[mask, 'Date'], df.loc[mask, 'margin'],
                          color=palette[s % len(palette)], s=8, alpha=0.55,
                          label=f'State {s}: μ={sorted_means[s]:.1f} USD/bbl')
        ax_ts.set(ylabel=f'{self.slate} margin (USD/bbl)')
        ax_ts.set_title(f'HMM regimes on {self.slate}/{self.region} crack — {N_STATES} states',
                        loc='left')
        ax_ts.legend(loc='upper left', fontsize=9)

        ax_st.plot(df['Date'], df['state'], color=self.COLORS['REF'],
                   linewidth=self.LW, drawstyle='steps-post')
        ax_st.set(xlabel='Date', ylabel='state',
                  yticks=list(range(N_STATES)))
    ...
    def forecast_next_day_regime(self, HORIZON=5):
        ...
        p = np.zeros(n); p[last_state] = 1.0
        latest_date = self.df.iloc[-1]['Date']
        print(f'HMM next-day regime forecast — as of {latest_date.date()}:')
        state_desc = '  ·  '.join([f'P(state{s})=_' for s in range(n)]).replace('_', '{:.3f}')
        for h in range(HORIZON + 1):
            tag = 'today' if h == 0 else f'+{h}d  '
            print(f'  {tag:6s} ' + '  ·  '.join(
                [f'P(state{s})={p[s]:.3f}' for s in range(n)]))
            p = p @ T
        return p
    ...
    def chart_seasonal_bootstrap_ci(self, SLATE=None, N_BOOT=2000):
        ...
        for m in range(1, 13):
            ...
            if len(obs) < 2:
                continue
            boot_means = np.array([rng.choice(obs, size=len(obs), replace=True).mean()
        ...
        ax.bar(out['month'], out['mean'], yerr=yerr,
               color=self.COLORS['MARGIN'], alpha=0.85, edgecolor='white',
               capsize=5, ecolor=self.COLORS['REF'],
               label=f'Monthly mean ± 95% bootstrap CI (n≈{out["n"].mean():.0f}/month)')
        full_mean = df['margin'].mean()
        ax.axhline(full_mean, color=self.COLORS['REF'], linestyle='--',
                   linewidth=0.8, label=f'IS mean = {full_mean:.2f}')
        ax.set_xticks(range(1, 13))
        ax.set_xticklabels(['Jan','Feb','Mar','Apr','May','Jun',
                            'Jul','Aug','Sep','Oct','Nov','Dec'])
        ax.set(xlabel='Calendar month', ylabel=f'{slate} margin (USD/bbl)')
        ax.set_title(f'Seasonality with 95% bootstrap CIs — {slate}/{self.region} (IS only)',
                     loc='left')
        ax.legend(loc='upper left', fontsize=9)

        crosses_zero = ((out['lo95'] < 0) & (out['hi95'] > 0)).sum()
        ci_widths = (out['hi95'] - out['lo95']).mean()
        print(f'Bootstrap CIs:  {crosses_zero}/12 months span zero · '
    ...
    def chart_opec_crosscheck(self):
        ...
        ax_ts.set_title('Cross-check — OPEC ASB vs our computed NWE ULSD crack',
        ...
        if len(merged) >= 2:
            ...
            ax_sc.scatter(merged['value_usd_bbl'], merged['Brent_ULSD_NWE_Crack'],
                          s=40, alpha=self.A['SCATTER'], color=self.COLORS['SCATTER'],
                          edgecolor='white')
