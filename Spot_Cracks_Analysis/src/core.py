...
DATAS_ROOT       = PORTFOLIO_ROOT / 'Datas'
...
class DataLoader:
    ...
    def brent_futures_strip(self):
        ...
        keep = ['Date'] + [f'Brent_M{n}_bbl' for n in range(1, 13) if f'Brent_M{n}_bbl' in df.columns]
        return df[keep]
    ...
    def eu_stocks_by_country(self, WHICH='crude'):
        ...
        mapping = {
            'crude':      'Raw_datas/Stocks/EU_Crude_Stocks_by_country.xlsx',
            'products':   'Raw_datas/Stocks/EU_Oil_Products_Stocks_by_country.xlsx',
            'feedstocks': 'Raw_datas/Stocks/EU_Refinery_Feedstocks_by_country.xlsx',
        }
        raw = pd.read_excel(self.DATA_ROOT / mapping[WHICH],
    ...
    def europe_diesel_import(self, GRANULARITY='monthly'):
        key = 'EUROPE_DIESEL_IMPORT_MONTHLY' if GRANULARITY == 'monthly' else 'EUROPE_DIESEL_IMPORT_5Y'
    ...
    def eu_refinery_intake(self):
        ...
        df = df[~df['Country'].str.contains('European Union|Euro area',
                                             na=False, regex=True)]
    ...
    def eu_consumption(self, WHICH='distillate'):
        ...
        key = {'distillate': 'EU_CONSUMPTION_DISTIL',
               'jet':        'EU_CONSUMPTION_JET',
               'products':   'EU_CONSUMPTION_PRODUCTS'}[WHICH]
...
class Chartable:
    ...
    def _init_styling(self, cfg, PROJECT_ROOT, SAVE_FIGURES=True):
        ...
        self.OUTPUT_DIR = (PROJECT_ROOT / cfg['PATHS']['FIGURES']
    ...
    def _add_event_markers(self, ax, events=None, y_top=None):
        ...
        for j, (d, label) in enumerate(events):
            dt = pd.to_datetime(d)
            if d0 is not None and dt.date() < d0: continue
            if d1 is not None and dt.date() > d1: continue
