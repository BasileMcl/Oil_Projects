...
class RegionalAnalysis(Chartable):
    ...
    def load(self):
        ...
        print(f'[Regional] Spot {len(self.spot)} | FX {len(self.fx)} | '
    ...
    def chart_med_nwe_integration(self):
        ...
        ax_top.plot(s['Date'], s['ULSD_NWE_bbl'], color=self.COLORS['PRICE'],
                    linewidth=self.LW, label='ULSD CIF NWE (USD/bbl)')
        ax_top.plot(s['Date'], s['ULSD_Med_bbl'], color=self.COLORS['CRACK'],
                    linewidth=self.LW, label='ULSD CIF Med (USD/bbl)')
        ax_top.set(ylabel='USD/bbl')
        ax_top.set_title('ULSD NWE vs ULSD Med — level',
                         loc='left', fontsize=10, fontweight='normal')
        ax_top.legend(loc='upper left')
        self._format_xaxis(ax_top)

        s['diff'] = s['ULSD_NWE_bbl'] - s['ULSD_Med_bbl']
        ax_bot.plot(s['Date'], s['diff'], color=self.COLORS['MARGIN'],
                    linewidth=self.LW)
        ...
        print(f'Mean NWE − Med:  {s["diff"].mean():+.2f} USD/bbl  '
              f'(σ = {s["diff"].std():.2f})')

        fig.suptitle('ULSD — Med / NWE integration',
    ...
    def chart_brent_wti_spread(self):
        ...
        ax_ts.plot(s['Date'], s['spread'], color=self.COLORS['PRICE'],
                   linewidth=self.LW)
        self._zero_line(ax_ts)
        mean_s = s['spread'].mean()
        ax_ts.axhline(mean_s, color=self.COLORS['REF'], linestyle=':',
                      linewidth=0.8, label=f'Mean = {mean_s:+.2f} USD/bbl')
        ax_ts.set(xlabel='Date', ylabel='Brent − WTI (USD/bbl)')
        ax_ts.set_title('Trans-Atlantic basin — Brent premium over WTI',
                        loc='left', fontsize=10, fontweight='normal')
        ax_ts.legend(loc='upper left')
        self._format_xaxis(ax_ts)
        self._add_event_markers(ax_ts)

        ax_hist.hist(s['spread'], bins=40, color=self.COLORS['PRICE'],
                     alpha=0.75, edgecolor='white', orientation='horizontal')
        ax_hist.axhline(mean_s, color=self.COLORS['REF'], linestyle=':',
                        linewidth=0.8)
        ...
        fig.suptitle('Brent ↔ WTI spot spread',
    ...
    def chart_eurusd_impact(self):
        ...
        ax_lvl.plot(df['Date'], df['fx'], color=self.COLORS['FX'],
                    linewidth=self.LW, label='EUR/USD')
        self._color_stack(ax_lvl, 'y', self.COLORS['FX'], 'EUR/USD')
        ax2 = ax_lvl.twinx()
        ax2.plot(df['Date'], df['crack'], color=self.COLORS['CRACK'],
                 linewidth=self.LW, alpha=0.85, label='Brent-ULSD NWE crack')
        self._color_stack(ax2, 'y', self.COLORS['CRACK'], 'Crack (USD/bbl)')
        ax_lvl.set_title('EUR/USD vs Brent-ULSD NWE crack',
                         loc='left', fontsize=10, fontweight='normal')
        self._format_xaxis(ax_lvl)

        ax_corr.plot(df['Date'], df['roll_r'], color=self.COLORS['PRICE'],
                     linewidth=self.LW)
        self._zero_line(ax_corr)
        ax_corr.set(xlabel='Date',
                    ylabel=f'{self.ROLL_WINDOW}-day Δ corr',
                    ylim=(-1.05, 1.05))
        ax_corr.set_title('Rolling ΔEUR/USD ↔ ΔCrack correlation',
                          loc='left', fontsize=10, fontweight='normal')
        ...
        cutoff = _pd.Timestamp(self.Z_IS_END if hasattr(self, 'Z_IS_END')
                                else '2024-12-31')
        d_is  = df[df['Date'] <= cutoff]
        d_oos = df[df['Date'] >  cutoff]
        if len(d_is) > 20:
            r_is = d_is['d_fx'].corr(d_is['d_crack'])
            print(f'  → IS window ({d_is["Date"].min().date()} → {cutoff.date()}): '
                  f'Δ-corr {r_is:+.3f}, n = {len(d_is)}')
        if len(d_oos) > 20:
            r_oos = d_oos['d_fx'].corr(d_oos['d_crack'])
            print(f'  → OOS window ({cutoff.date()} → {d_oos["Date"].max().date()}): '
        ...
        fig.suptitle('EUR/USD ↔ European refining margin',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_russia_export(self, TOP_N=8):
        ...
        df = self.ru_exp.copy()
        mon = (df.groupby(['Date', 'Destination'], as_index=False)['Quantity_kt']
                 .sum())
        top_dest = (mon.groupby('Destination')['Quantity_kt'].sum()
                       .sort_values(ascending=False).head(TOP_N).index)
        mon_top = mon[mon['Destination'].isin(top_dest)]
        pivot = mon_top.pivot(index='Date', columns='Destination',
                              values='Quantity_kt').fillna(0)
        ...
        ax_ts.set_title('Russia diesel exports — top destinations',
                        loc='left', fontsize=10, fontweight='normal')
        ...
        ax_pie.pie(latest.values, labels=latest.index, autopct='%.0f%%',
                   colors=colors[:len(latest)], startangle=90,
                   textprops={'fontsize': 8})
        ax_pie.set_title(f'Latest month: {pivot.index[-1].date()}',
                         loc='center', fontsize=10)

        total_latest = latest.sum()
        top1 = latest.index[0]
        print(f'Russia diesel exports latest month total: {total_latest:,.0f} kt')
        print(f'Top destination: {top1} ({latest.iloc[0]:,.0f} kt, '
              f'{100*latest.iloc[0]/total_latest:.0f}%)')
        fig.suptitle('Russia diesel export destinations',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=1.02)
    ...
    def chart_europe_import_mix(self, TOP_N=8):
        ...
        df = self.eu_imp.copy()
        top_origin = (df.groupby('Origin')['Quantity_kt'].sum()
                        .sort_values(ascending=False).head(TOP_N).index)
        df_top = df[df['Origin'].isin(top_origin)]
        pivot = (df_top.pivot_table(index='Date', columns='Origin',
                                    values='Quantity_kt', aggfunc='sum')
                       .fillna(0))
        ...
        ax.set_title('Europe diesel imports — top origins',
                     loc='left')
    ...
    def chart_diesel_gas_premium(self):
        ...
        ax_ts.plot(c['Date'], c['premium'], color=self.COLORS['MARGIN'],
                   linewidth=self.LW)
        self._zero_line(ax_ts)
        mean_p = c['premium'].mean()
        ax_ts.axhline(mean_p, color=self.COLORS['REF'], linestyle=':',
                      linewidth=0.8, label=f'Mean = {mean_p:+.2f} USD/bbl')
        ax_ts.set(xlabel='Date', ylabel='ULSD crack − EBOB crack (USD/bbl)')
        ax_ts.set_title('Diesel premium to gasoline',
                        loc='left', fontsize=10, fontweight='normal')
        ax_ts.legend(loc='upper left')
        self._format_xaxis(ax_ts)

        ax_hist.hist(c['premium'], bins=40, color=self.COLORS['MARGIN'],
                     alpha=0.75, edgecolor='white', orientation='horizontal')
        ax_hist.axhline(mean_p, color=self.COLORS['REF'], linestyle=':',
                        linewidth=0.8)
        ax_hist.set(xlabel='Frequency', title='Distribution')
        ax_hist.grid(True, alpha=0.3, axis='x')

        peak = c.loc[c['premium'].idxmax()]
        print(f'Diesel-gas premium: mean {mean_p:+.2f} USD/bbl, '
              f'peak {peak["premium"]:+.2f} on {peak["Date"].date()}')
        fig.suptitle('Diesel premium over gasoline',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=1.02)
    ...
    def chart_opec_trade_flows(self, TOP_N=12):
        ...
        crude = self.opec_crude_exp[self.opec_crude_exp['is_aggregate'] == False].copy()
        prod  = self.opec_prod_exp[self.opec_prod_exp['is_aggregate'] == False].copy()
        latest_year = max(crude['Year'].max(), prod['Year'].max())

        fig, axes = plt.subplots(2, 2, figsize=(self.SIZES['STANDARD'][0] + 2,
                                                 self.SIZES['STANDARD'][1] + 3),
        ...
        for row, (df, label, color) in enumerate([
            ...
            # Latest-year ranking
            rank = (df[df['Year'] == latest_year]
                    .sort_values('Value_kbd', ascending=False)
                    .head(TOP_N))
            ax_bar = axes[row, 0]
            ax_bar.barh(rank['Country'][::-1], rank['Value_kbd'][::-1],
            ...
            for i, c in enumerate(top_countries):
                ...
                ax_ts.plot(sub['Year'], sub['Value_kbd'], color=colors_c[i % 10],
                           linewidth=self.LW, label=c, marker='o', markersize=3)
            ax_ts.set(xlabel='Year', ylabel='kbd',
                      title=f'{label.capitalize()} exports — 10y trajectory')
            ax_ts.legend(loc='center left', bbox_to_anchor=(1.01, 0.5),
                         fontsize=7, ncol=1)
            ax_ts.grid(True, alpha=0.3)

        # Summary prints
        c_top = (crude[crude['Year'] == latest_year]
                 .sort_values('Value_kbd', ascending=False).head(3))
        p_top = (prod[prod['Year'] == latest_year]
                 .sort_values('Value_kbd', ascending=False).head(3))
        ...
        fig.suptitle(f'Global oil trade — crude & product exports (OPEC ASB, through {latest_year})',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
    ...
    def chart_opec_tanker_freight(self):
        ...
        for i, route in enumerate(sorted(clean['route'].unique())):
            ...
            ax_c.plot(sub['year'], sub['value'], color=colors_c[i % 10],
                      linewidth=self.LW, marker='o', markersize=3, label=route)
        ax_c.set(ylabel='WS%  (clean)')
        ax_c.set_title('Tanker freight — clean (products) routes',
                       loc='left', fontsize=10, fontweight='normal')
        ...
        for i, route in enumerate(sorted(dirty['route'].unique())):
            ...
            ax_d.plot(sub['year'], sub['value'], color=colors_c[i % 10],
                      linewidth=self.LW, marker='o', markersize=3, label=route)
        ax_d.set(xlabel='Year', ylabel='WS%  (dirty)')
        ax_d.set_title('Tanker freight — dirty (crude) routes',
                       loc='left', fontsize=10, fontweight='normal')
        ...
        for ax in (ax_c, ax_d):
            for y, lbl in [(2022, 'Russia invasion / sanctions'),
                           (2024, 'Red Sea attacks')]:
        ...
        fig.suptitle('Tanker freight — WS% assessments (OPEC ASB, annual averages)',
                     x=0.01, ha='left', fontsize=13, fontweight='bold', y=0.995)
